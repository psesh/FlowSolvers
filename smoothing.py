...
def smooth(flow_property, corrected_flow_property, boundary_conditions, grid_parameters):
    ...
    for i in range(0, nu):
        ...
        for j in range(2, nv - 1):
            ...
            corrected_new = fcorrection  * (  flow_property[j,i,0] - avg)
            corrected_flow_propery[j, i, 0] = 0.88 * corrected_flow_propery[j, i, 0] + 0.01 * corrected_new
            store[j, i, 0] = smooth_fac_minus_one * flow_property[j, i, 0] + smooth_fac_input * (average + corrected_flow_propery[j, i, 0])
        ...
        corrected_flow_propery[0, i, 0] = 0.99 * corrected_flow_propery[0, i, 0] + 0.01 * corrected_new
        ...
        corrected_flow_propery[nv-1, i, 0] = 0.99 * corrected_flow_propery[nv-1, i, 0] + 0.01 * corrected_new

        # Smooth the surface values!
        store[0, i, 0] = smooth_fac_minus_one * flow_property[0,i,0] + smooth_fac_input * (average0 + corrected_flow_propery[0, i, 0] )
        store[nv-1,i,0] = smooth_fac_minus_one * flow_property[nv-1,i,0] + smooth_fac_input * (averagenv + corrected_flow_propery[nv-1, i, 0] )
