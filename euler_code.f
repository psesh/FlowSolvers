...
C
      call set_fluxes
C
      call sum_fluxes (fluxi_mass,fluxj_mass,ro,delro)
      call sum_fluxes (fluxi_enth,fluxj_enth,roe,delroe)
      call sum_fluxes (fluxi_xmom,fluxj_xmom,rovx,delrovx)
      call sum_fluxes (fluxi_ymom,fluxj_ymom,rovy,delrovy)
C
C
      call smooth(ro)
      call smooth(rovx)
...
C  Assign Fortran unit 2 to file 'flow'
      OPEN(UNIT=1, FILE='test1_geom')
      OPEN(UNIT=2, FILE='test1_flow')
C
C
C
...
C
      REAL XO,YO,DEL_Y,DEL_X,A1,A2,B1,B2,DMIN_I,DMIN_J,DMIN_VAL,C1,C2
c      WRITE(*,*) NI,NJ
       DO I = 1,NI
...
C
      ...
      DO I = 1,NI
         DO J = 1,NJ-1
          ...
          ZLENI= (DLIX(I,J)**2+DLIY(I,J)**2)**0.5
          DMIN_I = MIN( DMIN_I, ZLENI)
         END DO
      END DO 
      ...
      DO I = 1,NI-1
 	  ...
 	  ZLENJ= (DLJX(I,J)**2+DLJY(I,J)**2)**0.5
          DMIN_J = MIN( DMIN_J, ZLENJ)
        END DO
      END DO

       DMIN = MIN(DMIN_I, DMIN_J)
	PRINT*, DMIN
c       WRITE(*,*) DMIN
...
C
      ...
      UMAX   = ASTAG
C
C      INSERT ONE LINE OF CODE HERE TO CALCULATE "deltat".
C
...
C**************************************************************************
C
      subroutine sum_fluxes (iflux,jflux,prop,delprop)
C
